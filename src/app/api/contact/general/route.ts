...
import { z } from 'zod';
import { generalSchema } from '@/lib/validations/contact.schemas';
...
export async function POST(req: Request) {
  try {
    ...
    const { name, email, message } = generalSchema.parse(body);

    const res = await fetch(`${BACKEND}/api/contact/general`, {
      method: 'POST',
